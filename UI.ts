...
@Component({
  ...
  template: `
    <div class="min-h-screen bg-gray-50 flex flex-col lg:flex-row">
      ...
      <aside class="w-full lg:w-80 bg-white p-6 shadow-xl border-r border-gray-200 flex-shrink-0">
        <h1 class="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 to-purple-600 mb-6">Concept Canvas</h1>
        
        <!-- User/Auth Status -->
        <div class="mb-6 p-3 bg-indigo-50 rounded-lg text-sm border border-indigo-200">
        ...
        <!-- Card Creation -->
        @if (!editingCard()) {
          <div class="space-y-4 mb-8 p-4 bg-gray-50 rounded-xl border">
        ...
        @if (editingCard(); as card) {
          <div class="space-y-4 mb-8 p-4 border border-indigo-300 rounded-xl bg-indigo-50 shadow-md">
        ...
        <div class="space-y-4 p-4 bg-gray-50 rounded-xl border">
          ...
          <div class="flex flex-col space-y-2 text-sm">
            <div class="p-2 border rounded-md font-medium" [class]="selectedCardIds().length > 0 ? 'bg-yellow-50 border-yellow-300 text-yellow-800' : 'bg-gray-100 border-gray-200 text-gray-600'">
              Selected: <span class="font-bold">{{ selectedCardIds().length }}</span> / 2
            </div>
            
            <!-- Connection Label Selector -->
            <label class="block">
                ...
                <select #connectionLabel class="block w-full rounded-md border-gray-300 shadow-sm p-2 border mt-1">
                  <option value="Related To">Related To (Standard)</option>
      ...
      <main class="flex-grow relative overflow-hidden bg-gray-100">
        ...
        <!-- Zoom Controls (NEW) -->
        <div class="absolute top-4 right-4 z-30 flex space-x-2 p-2 bg-white rounded-lg shadow-lg">
            <button (click)="zoomCanvas(1.1)" class="p-2 rounded-lg bg-indigo-100 text-indigo-700 hover:bg-indigo-200 transition">
            ...
            <button (click)="zoomCanvas(0.9)" class="p-2 rounded-lg bg-indigo-100 text-indigo-700 hover:bg-indigo-200 transition">
            ...
            <button (click)="resetView()" class="p-2 rounded-lg bg-indigo-100 text-indigo-700 hover:bg-indigo-200 transition">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.962 8.962 0 0120 12c0 2.47-.9 4.745-2.4 6.4M12 20v-5h-.582m-15.356-2A8.962 8.962 0 014 12c0-2.47.9-4.745 2.4-6.4"></path></svg>
            </button>
        </div>
        ...
        <svg [attr.width]="canvasWidth()" [attr.height]="canvasHeight()" 
             ...
             [style.transform]="'translate(' + panOffset().x + 'px, ' + panOffset().y + 'px) scale(' + scaleFactor() + ')'"
          ...
          <defs>
            <pattern id="grid" [attr.width]="gridSize * scaleFactor()" [attr.height]="gridSize * scaleFactor()" patternUnits="userSpaceOnUse">
              <path [attr.d]="'M ' + gridSize * scaleFactor() + ' 0 L 0 0 0 ' + gridSize * scaleFactor()" fill="none" stroke="#e0e7ff" stroke-width="1"/>
            </pattern>
            <!-- Define Semantic Line Styles -->
            <line id="dash-related" stroke-dasharray="5, 5" />
            <line id="dash-prerequisite" stroke-dasharray="10, 2" />
            <line id="dash-contradicts" stroke-dasharray="3, 3" />
          </defs>
          <rect [attr.width]="canvasWidth() / scaleFactor()" [attr.height]="canvasHeight() / scaleFactor()" fill="url(#grid)" 
                [attr.transform]="'scale(' + (1 / scaleFactor()) + ')'"/>
          ...
          @for (connection of allConnections(); track connection.id) {
            @if (getConnectionCoordinates(connection); as coords) {
              <line 
                ...
                [attr.stroke-dasharray]="getLineStyle(connection.label)"
  ...
  styles: [`
    ...
    .card {
        /* Enhance click/drag visual feedback */
        transition: transform 0.1s ease-out, box-shadow 0.15s ease-out, ring-color 0.15s ease-out;
    }
...
export class App {
  ...
  async addCard(title: string, content: string, color: string) {
    ...
    const newCard: LearningCard = {
      ...
      x: 50 + (this.panOffset().x / this.scaleFactor()), // Adjust initial position based on current pan/zoom
      y: 50 + (this.panOffset().y / this.scaleFactor()),
      color: color || '#a5b4fc',
      ownerId: this.userId()!
    };
  ...
  dragMove(event: MouseEvent | TouchEvent) {
    ...
    // Calculate new position relative to the container and factor in pan/zoom
    let newX = clientX - containerRect.left;
    let newY = clientY - containerRect.top;
    
    // Reverse the pan and zoom transformations
    newX = (newX - this.panOffset().x) / this.scaleFactor() - this.dragOffset().x;
    newY = (newY - this.panOffset().y) / this.scaleFactor() - this.dragOffset().y;
  ...
  zoomCanvas(factor: number) {
    this.scaleFactor.update(s => Math.min(Math.max(s * factor, 0.5), 2.5)); // Limit zoom range
  }
  ...
  panStart(event: MouseEvent) {
    // Only pan if not dragging a card (left mouse button)
    if (this.activeCardId() === null && event.button === 0) {
  ...
  onWheel(event: WheelEvent) {
    event.preventDefault();
    // Only zoom using the mouse wheel if Ctrl/Cmd key is pressed
  ...
  /** Determines the line style (dash pattern) based on the relationship label. (NEW) */
  getLineStyle(label: string): string {
    switch (label) {
      case 'Prerequisite':
        return '8, 4'; // Dotted line for sequence
      case 'Contradicts':
        return '3, 3, 1, 3'; // Short dashes for conflict
      case 'Example Of':
        return '15, 3'; // Long dash for direct example
      default: // Related To, or anything else
        return ''; // Solid line
    }
  }
