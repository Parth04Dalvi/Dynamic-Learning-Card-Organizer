      ...
      <aside class="w-full lg:w-80 bg-white p-6 shadow-xl border-r border-gray-200 flex-shrink-0">
        ...
        <div class="space-y-4">
          ...
          <div class="flex flex-col space-y-2 text-sm">
            ...
            <input #connectionLabel type="text" placeholder="e.g., 'Depends on'" class="block w-full rounded-md border-gray-300 shadow-sm p-2 border">
            
            <button 
              (click)="createConnection(connectionLabel.value); connectionLabel.value='';"
          ...
          @for (connection of allConnections(); track connection.id) {
            @if (getConnectionCoordinates(connection); as coords) {
              ...
              <text [attr.x]="(coords.x1 + coords.x2) / 2" [attr.y]="(coords.y1 + coords.y2) / 2 - 5"
                    text-anchor="middle" font-size="12" fill="#4f46e5" class="pointer-events-none font-sans">
                {{ connection.label }}
              </text>
            }
          }
        ...
        <div id="cards-container" 
          ...
          @for (card of allCards(); track card.id) {
            <div 
              ...
              [class.ring-4]="selectedCardIds().includes(card.id)"
